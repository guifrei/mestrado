...
\end{align} 
onde $\mathbb{A}_{j,m}, \mathbb{B}_{j,m}, \mathbb{C}_{j,m}$ e $\mathbb{D}_{j,m}$ são constantes a serem determinadas através das condições de contorno sobre as fronteiras $\Gamma_0$ e $\Gamma_\infty$, e sobre a superfície de contato $\Gamma$\footnote{A expressão \eqref{solucao_temporaria_F1}, através da expansão dos argumentos das funções hiberbólicas, pode ser reescrita como:
