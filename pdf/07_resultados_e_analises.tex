...
Na prática, porém, a função $Y(x)$ é desconhecida, e dispõe-se apenas de dados discretos, correspondentes às temperaturas medidas em pontos distintos da superfície superior. No trabalho de \cite{tese_padilha}, foi feita uma aproximação dessa função $Y(x)$ através de uma \textit{spline} que interpolava essas medidas. Naquele mesmo trabalho, foi comentada a possibilidade de se interpolar as medidas sintéticas por meio de uma expansão em série de funções ortogonais às autofunções do problema de autovalor. Ou seja, a função $Y(x)$ poderia ser escrita na forma
\begin{align}
