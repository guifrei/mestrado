...
\begin{table}[h!b]
	\begin{center}
		\begin{tabular}{@{}cc@{}}
			...
			2     & $\displaystyle\frac{b}{2} + \frac{1}{40} \sin\frac{2\pi x}{a} + \frac{1}{20}\cos\frac{4\pi x}{a}$     \\ \\
			3       & $\displaystyle \frac{b}{2} + \frac{1}{20} \cos\frac{4 \pi  x}{a}$ \\
			\bottomrule
		\end{tabular}		
	\end{center}
...
%\end{align}
%
%É importante lembrar que $\beta_j$ e $\gamma_j$ são funções de $x$, de modo que as estimativas de salto de temperatura e fluxo de calor devem ser calculadas em cada ponto de interesse $x$, sendo $0 \le x \le a$. A condutância térmica de contato, por sua vez, é a razão entre o fluxo de calor e o salto de temperatura na interface, calculados ponto a ponto:
%\begin{align}
%& h_c(x) % = \frac{- k_1 \displaystyle\frac{\partial T_1}{\partial\mathbf{n_1}}\bigg|_\Gamma}{[T_1 - T_2]_\Gamma} 
%= \frac{\displaystyle- k_1 \frac{\partial T_1}{\partial\mathbf{n_1}}\bigg|_\Gamma}{\displaystyle[T_1 - T_2]_\Gamma} \label{expressao_final_ctc_repetida}
%\end{align}
%
%Em teoria, quanto maiores fossem os limites dos somatórios $N_1$ e $N_2$, melhores seriam as estimativas fornecidas de salto de temperatura e fluxo de calor. No desenvolvimento deste trabalho, porém, observou-se que esse padrão era seguido até algum número máximo de termos nos somatórios \eqref{resultado_1_again} e \eqref{resultado_2_again}, a partir do qual as estimativas afastavam-se cada vez mais dos valores exatos correspondentes. Ou seja, ao contrário do que normalmente se observa numa série numérica, um incremento na quantidade de parcelas a serem somadas naquelas expressões não implicava obrigatoriamente numa tendência consistente de convergência do somatório para um valor finito. Em outras palavras, os somatórios que representam o salto de temperatura na interface e o fluxo de calor na interface têm comportamento \textit{semi-convergente}.
%
%É necessário, portanto, estabelecer um critério para determinação do número máximo de parcelas nos somatórios que representavam as estimativas. Para levantamento desse número ótimo de termos, \cite{tese_padilha} empregou, como métrica de avaliação da qualidade da estimativa, uma norma baseada nas diferenças entre duas estimativas sucessivas ao longo da superfície $\Gamma$:
...
%
%Outra métrica estabelecida e analisada por \cite{tese_padilha} foi a norma acumulada, definida por:
%\begin{align}
...
%Este trabalho propõe uma forma diferente de se avaliar a semi-convergência das aproximações representadas por \eqref{resultado_1_again} e \eqref{resultado_2_again}. Deve-se observar primeiro que os termos $\Re(F_{1,j})$ e $\Re(G_{1,j})$ têm um comportamento distinto dos termos $\beta_j(x)$ e $\gamma_j(x)$, à medida em que se varia o valor do índice $j$. Sejam, por exemplo, os valores médios $\bar{\beta}_j$ e $\bar{\gamma}_j$, definidos por:
%\begin{align}
%& \bar{\beta}_j = \frac{1}{a}\int_0^a \beta_j(x) dx \\ \nonumber \\
%& \bar{\gamma}_j = \frac{1}{a}\int_0^a \gamma_j(x) dx
%\end{align}
%
%\begin{figure}[h!b]
%	\centering		
...
%		\pgfplotstableread{../data/media_beta_gamma_interface_01.dat} 
%		\teff
...
%	ylabel= $J$,
%	ylabel style={rotate=-90, at={(-0.1, 1)}, anchor = south west}]
%	\pgfplotstableread{../data/metricas_interface_01_conductance_01_stdev_00.dat} 
%	\teff
%	\addplot[color=blue,mark=o,mark options={mark size=2.0pt}] table[x index = {0}, y index = {2}] from \teff;	
%	\end{axis}
%	\end{tikzpicture}
%	\caption*{AAA}
%\end{minipage}
...
\begin{figure}[h!b]
	\graficoestimativa{fluxo_calor}{1}{1}{11}{03}{03}{a}{-k_1 \frac{\partial T_1}{\partial\mathbf{n}_1}\big|_{\Gamma}}{W/$\text{m}^2$}
	\graficoestimativa{fluxo_calor}{1}{2}{11}{06}{03}{a}{-k_1 \frac{\partial T_1}{\partial\mathbf{n}_1}\big|_{\Gamma}}{W/$\text{m}^2$}
	\graficoestimativa{fluxo_calor}{1}{3}{08}{06}{04}{a}{-k_1 \frac{\partial T_1}{\partial\mathbf{n}_1}\big|_{\Gamma}}{W/$\text{m}^2$}
...
\begin{figure}[h!b]
	\graficoestimativa{delta_temperatura}{2}{1}{15}{07}{08}{a}{\Delta T\big|_{\Gamma}}{\celsius}
	\graficoestimativa{delta_temperatura}{2}{2}{15}{05}{03}{a}{\Delta T\big|_{\Gamma}}{\celsius}
...
%
\begin{figure}[h!b]
	\graficoestimativa{fluxo_calor}{2}{1}{05}{05}{03}{a}{-k_1 \frac{\partial T_1}{\partial\mathbf{n}_1}\big|_{\Gamma}}{W/$\text{m}^2$}
	\graficoestimativa{fluxo_calor}{2}{2}{03}{03}{03}{a}{-k_1 \frac{\partial T_1}{\partial\mathbf{n}_1}\big|_{\Gamma}}{W/$\text{m}^2$}
	\graficoestimativa{fluxo_calor}{2}{3}{04}{04}{05}{a}{-k_1 \frac{\partial T_1}{\partial\mathbf{n}_1}\big|_{\Gamma}}{W/$\text{m}^2$}
...
\newcommand{\graficosmetricas}[4]{%
	\begin{minipage}[t][6cm][c]{4.5cm}
		...
		ylabel= $T_1\big|_{\Gamma_0}$ (\celsius),
		ylabel style={rotate=-90, at={(-0.1, 1)}, anchor = south west}]			
		\addplot[color=blue,mark=triangle,mark options={mark size=2.0pt}] table[x index=0,y index=#4] {../data/metricas_interface_0#1_conductance_0#2_stdev_0#3.dat};				
		\end{axis}
		\end{tikzpicture}
		%\caption*{(#3) Perfil #2}
	\end{minipage}
