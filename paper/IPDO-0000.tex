...
If heat transfer takes place through a common boundary between two bodies in physical contact, a discontinuity in the temperature profile is observed across the contact interface, which indicates that the thermal contact is not perfect there. This phenomenon basically occurs due to the presence of roughness or imperfections of the materials along the contact interface at the microscopic level. In most cases, the space between the bodies can be considered small enough to avoid convection and radiation effects. Under these conditions, the \textit{thermal contact conductance} (TCC) is defined as the ratio of the heat flux per area unit to the temperature drop, both evaluated at that interface:

\begin{equation}
h_c = \frac{q_c}{\Delta T_c} \label{definicao_1}
\end{equation}
where $q_c$ is the heat flux and $\Delta T_c$ is the temperature jump, both mesured across the contact interface. The reciprocical of the TCC is known as \textit{thermal contact resistance} (TCR):
\begin{equation}
R_c = \frac{1}{h_c} = \frac{\Delta T_c}{q_c} \label{definicao_1a}
\end{equation}
...
The TCC may be seen as a parameter to evaluate the quality of the contact between two solid bodies,
and is basically a thermal approach for a mechanical problem (indirect qualitative evaluation of imperfections due to small roughness on the surfaces in contact). In extreme cases, the TCC vanishes on perfect thermally insulated regions along the contact interface, since the heat flux on the numerator in equation \eqref{definicao_1} will be zero, whereas an infinite value for TCC will be found where the contact itself is perfect, for which the temperature jump on the denominator in \eqref{definicao_1} will be zero. Some examples of the necessity of calculating or estimating the TCC may be found in aerodynamics\cite{artigo_aerospacial}, microelectronics\cite{artigo_snaith} or heat exchanger projects\cite{artigo_huang}. 
...
The method of the Reciprocity Functionals, proposed by by Colaço and Alves\cite{reciproc_3}. has been applied in recent researches on indirect evaluation of heat flux and temperature jumps across the contact interface\cite{artigo_colaco_2, artigo_colaco_3, artigo_abreu_2,artigo_abreu_3, artigo_padilha_2}. This method was based on a work by Andrieux and Ben Abda\cite{artigo_andrieux}, as a tool for evaluating the presence of discontinuities or cracks inside homogeneous materials. According to these authors, if a set of boundary conditions are imposed on a homogeneous body, a scalar field (such as temperature) can be determined as a result of the solution of an associated elliptic problem (a heat conduction problem, in this particular case). If there are discontinuities or cracks inside the body, the same boundary conditions will lead to a different solution. Thus, inner features of the body may be inferred from measures taken along its boundary.

The use of the method of the Reciprocity Functionals resulted in the development of non-intrusive, non-iterative, and more computationally efficient approaches for estimation of the TCC. However, these techniques have been applied to regular geometrical configurations, such as rectangular or circular-shaped cross section contact interfaces. Nevertheless, the analytical development envolving the use of the Reciprocity Functional approach is not restricted to any particular geometry, and may be expanded to more complex configurations.
...
\noindent{\fontfamily{phv}\selectfont \textbf{ANALYTICAL FORMULATION FOR THE THERMAL CONTACT CONDUCTANCE}}
...
\noindent{\fontfamily{phv}\selectfont \textbf{Thermal contact conductance prediction}}
...
\noindent{\fontfamily{phv}\selectfont \textbf{Influence of the interface geometry}}

In order to analyse the influence of the interface geometry in the estimatives of the TCC, the same results of the previous section will be shown here in a different organization. For every TCC profile, the corresponding estimatives for each interface geometry will be plotted in the same graph, and grouped by the standard deviations used in the simulations.

Figures \ref{unlabeled_11}, \ref{unlabeled_21} e \ref{unlabeled_31} correspond to the case of measured temperatures without noise ($\sigma = \text{0.0}\celsius$). The results suggest that, for this specific condition, the interface geometry shows little influence in the obtained profiles.
...
\begin{figure}[H]
	...
	\teff
	\addplot[color=black, line width=1.5pt] table from \teff;
	\pgfplotstableread{../data/estimativa_ctc_interface_01_conductance_01_stdev_00.dat} 
	\teff
	\addplot[only marks, color=blue,mark=o,mark options={mark size=1.5pt}] table from \teff;
	\pgfplotstableread{../data/estimativa_ctc_interface_02_conductance_01_stdev_00.dat} 
	\teff
	\addplot[only marks,color=red,mark=square,mark options={mark size=1.5pt}] table from \teff;
	\pgfplotstableread{../data/estimativa_ctc_interface_03_conductance_01_stdev_00.dat} 
	\teff
	\addplot[only marks,color=gray,mark=triangle,mark options={mark size=1.5pt}] table from \teff;
	\end{axis}
	\end{tikzpicture}
	%\caption*{(#4) Profile #3}
	%\end{minipage}
	\caption{Comparison of estimated TCC profile 1 for interface geometry 1 ($\textcolor{blue}{\ocircle}$), interface geometry 2 ($\textcolor{red}{\square}$) and interface geometry 3 ($\textcolor{gray}{\triangle}$), and the exact profile ($\text{--}$), using $\sigma = \text{0.0}\celsius$}
	\label{unlabeled_11}
\end{figure}
...
\begin{figure}[H]
	...
	\teff
	\addplot[color=black, line width=1.5pt] table from \teff;
	\pgfplotstableread{../data/estimativa_ctc_interface_01_conductance_02_stdev_00.dat} 
	\teff
	\addplot[only marks, color=blue,mark=o,mark options={mark size=1.5pt}] table from \teff;
	\pgfplotstableread{../data/estimativa_ctc_interface_02_conductance_02_stdev_00.dat} 
	\teff
	\addplot[only marks,color=red,mark=square,mark options={mark size=1.5pt}] table from \teff;
	\pgfplotstableread{../data/estimativa_ctc_interface_03_conductance_02_stdev_00.dat} 
	\teff
	\addplot[only marks,color=gray,mark=triangle,mark options={mark size=1.5pt}] table from \teff;
	\end{axis}
	\end{tikzpicture}
	%\caption*{(#4) Profile #3}
	%\end{minipage}
	\caption{Comparison of estimated TCC profile 2 for interface geometry 1 ($\textcolor{blue}{\ocircle}$), interface geometry 2 ($\textcolor{red}{\square}$) and interface geometry 3 ($\textcolor{gray}{\triangle}$), and the exact profile ($\text{--}$), using $\sigma = \text{0.0}\celsius$}
	\label{unlabeled_21}
\end{figure}
...
\begin{figure}[H]
	...
	\teff
	\addplot[color=black, line width=1.5pt] table from \teff;
	\pgfplotstableread{../data/estimativa_ctc_interface_01_conductance_03_stdev_00.dat} 
	\teff
	\addplot[only marks, color=blue,mark=o,mark options={mark size=1.5pt}] table from \teff;
	\pgfplotstableread{../data/estimativa_ctc_interface_02_conductance_03_stdev_00.dat} 
	\teff
	\addplot[only marks,color=red,mark=square,mark options={mark size=1.5pt}] table from \teff;
	\pgfplotstableread{../data/estimativa_ctc_interface_03_conductance_03_stdev_00.dat} 
	\teff
	\addplot[only marks,color=gray,mark=triangle,mark options={mark size=1.5pt}] table from \teff;
	\end{axis}
	\end{tikzpicture}
	%\caption*{(#4) Profile #3}
	%\end{minipage}
	\caption{Comparison of estimated TCC profile 3 for interface geometry 1 ($\textcolor{blue}{\ocircle}$), interface geometry 2 ($\textcolor{red}{\square}$) and interface geometry 3 ($\textcolor{gray}{\triangle}$), and the exact profile ($\text{--}$), using $\sigma = \text{0.0}\celsius$}
	\label{unlabeled_31}
\end{figure}

%==================
Figures \ref{unlabeled_12}, \ref{unlabeled_22} e \ref{unlabeled_32} correspond to the case of measured temperatures with a noise level of $\text{0.1}\celsius$. As expected, the presence of noise in the measured temperatures along the interface $\Gamma_0$ has the effect of deviating the predictions from the theoretical profiles with no temperature measurement errors.
...
\begin{figure}[H]
	...
	\teff
	\addplot[color=black, line width=1.5pt] table from \teff;
	\pgfplotstableread{../data/estimativa_ctc_interface_01_conductance_01_stdev_01.dat} 
	\teff
	\addplot[only marks, color=blue,mark=o,mark options={mark size=1.5pt}] table from \teff;
	\pgfplotstableread{../data/estimativa_ctc_interface_02_conductance_01_stdev_01.dat} 
	\teff
	\addplot[only marks,color=red,mark=square,mark options={mark size=1.5pt}] table from \teff;
	\pgfplotstableread{../data/estimativa_ctc_interface_03_conductance_01_stdev_01.dat} 
	\teff
	\addplot[only marks,color=gray,mark=triangle,mark options={mark size=1.5pt}] table from \teff;
	\end{axis}
	\end{tikzpicture}
	%\caption*{(#4) Profile #3}
	%\end{minipage}
	\caption{Comparison of estimated TCC profile 1 for interface geometry 1 ($\textcolor{blue}{\ocircle}$), interface geometry 2 ($\textcolor{red}{\square}$) and interface geometry 3 ($\textcolor{gray}{\triangle}$), and the exact profile ($\text{--}$), using $\sigma = \text{0.1}\celsius$}
	\label{unlabeled_12}
\end{figure}
...
\begin{figure}[H]
	...
	\teff
	\addplot[color=black, line width=1.5pt] table from \teff;
	\pgfplotstableread{../data/estimativa_ctc_interface_01_conductance_02_stdev_01.dat} 
	\teff
	\addplot[only marks, color=blue,mark=o,mark options={mark size=1.5pt}] table from \teff;
	\pgfplotstableread{../data/estimativa_ctc_interface_02_conductance_02_stdev_01.dat} 
	\teff
	\addplot[only marks,color=red,mark=square,mark options={mark size=1.5pt}] table from \teff;
	\pgfplotstableread{../data/estimativa_ctc_interface_03_conductance_02_stdev_01.dat} 
	\teff
	\addplot[only marks,color=gray,mark=triangle,mark options={mark size=1.5pt}] table from \teff;
	\end{axis}
	\end{tikzpicture}
	%\caption*{(#4) Profile #3}
	%\end{minipage}
	\caption{Comparison of estimated TCC profile 2 for interface geometry 1 ($\textcolor{blue}{\ocircle}$), interface geometry 2 ($\textcolor{red}{\square}$) and interface geometry 3 ($\textcolor{gray}{\triangle}$), and the exact profile ($\text{--}$), using $\sigma = \text{0.1}\celsius$}
	\label{unlabeled_22}
\end{figure}
...
\begin{figure}[H]
	...
	\teff
	\addplot[color=black, line width=1.5pt] table from \teff;
	\pgfplotstableread{../data/estimativa_ctc_interface_01_conductance_03_stdev_01.dat} 
	\teff
	\addplot[only marks, color=blue,mark=o,mark options={mark size=1.5pt}] table from \teff;
	\pgfplotstableread{../data/estimativa_ctc_interface_02_conductance_03_stdev_01.dat} 
	\teff
	\addplot[only marks,color=red,mark=square,mark options={mark size=1.5pt}] table from \teff;
	\pgfplotstableread{../data/estimativa_ctc_interface_03_conductance_03_stdev_01.dat} 
	\teff
	\addplot[only marks,color=gray,mark=triangle,mark options={mark size=1.5pt}] table from \teff;
	\end{axis}
	\end{tikzpicture}
	%\caption*{(#4) Profile #3}
	%\end{minipage}
	\caption{Comparison of estimated TCC profile 3 for interface geometry 1 ($\textcolor{blue}{\ocircle}$), interface geometry 2 ($\textcolor{red}{\square}$) and interface geometry 3 ($\textcolor{gray}{\triangle}$), and the exact profile ($\text{--}$), using $\sigma = \text{0.1}\celsius$}
	\label{unlabeled_32}
\end{figure}

%==================

Figures \ref{unlabeled_13}, \ref{unlabeled_23} e \ref{unlabeled_33} correspond to the case of measured temperatures with a noise level of $\text{0.5}\celsius$. The predicted profiles still remain deviated from the expected ones, but the general behavior is preserved.
...
\begin{figure}[H]
	...
	\teff
	\addplot[color=black, line width=1.5pt] table from \teff;
	\pgfplotstableread{../data/estimativa_ctc_interface_01_conductance_01_stdev_05.dat} 
	\teff
	\addplot[only marks, color=blue,mark=o,mark options={mark size=1.5pt}] table from \teff;
	\pgfplotstableread{../data/estimativa_ctc_interface_02_conductance_01_stdev_05.dat} 
	\teff
	\addplot[only marks,color=red,mark=square,mark options={mark size=1.5pt}] table from \teff;
	\pgfplotstableread{../data/estimativa_ctc_interface_03_conductance_01_stdev_05.dat} 
	\teff
	\addplot[only marks,color=gray,mark=triangle,mark options={mark size=1.5pt}] table from \teff;
	\end{axis}
	\end{tikzpicture}
	%\caption*{(#4) Profile #3}
	%\end{minipage}
	\caption{Comparison of estimated TCC profile 1 for interface geometry 1 ($\textcolor{blue}{\ocircle}$), interface geometry 2 ($\textcolor{red}{\square}$) and interface geometry 3 ($\textcolor{gray}{\triangle}$), and the exact profile ($\text{--}$), using $\sigma = \text{0.5}\celsius$}
	\label{unlabeled_13}
\end{figure}
...
\begin{figure}[H]
	...
	\teff
	\addplot[color=black, line width=1.5pt] table from \teff;
	\pgfplotstableread{../data/estimativa_ctc_interface_01_conductance_02_stdev_05.dat} 
	\teff
	\addplot[only marks, color=blue,mark=o,mark options={mark size=1.5pt}] table from \teff;
	\pgfplotstableread{../data/estimativa_ctc_interface_02_conductance_02_stdev_05.dat} 
	\teff
	\addplot[only marks,color=red,mark=square,mark options={mark size=1.5pt}] table from \teff;
	\pgfplotstableread{../data/estimativa_ctc_interface_03_conductance_02_stdev_05.dat} 
	\teff
	\addplot[only marks,color=gray,mark=triangle,mark options={mark size=1.5pt}] table from \teff;
	\end{axis}
	\end{tikzpicture}
	%\caption*{(#4) Profile #3}
	%\end{minipage}
	\caption{Comparison of estimated TCC profile 2 for interface geometry 1 ($\textcolor{blue}{\ocircle}$), interface geometry 2 ($\textcolor{red}{\square}$) and interface geometry 3 ($\textcolor{gray}{\triangle}$), and the exact profile ($\text{--}$), using $\sigma = \text{0.5}\celsius$}
	\label{unlabeled_23}
\end{figure}
...
\begin{figure}[H]
	...
	\teff
	\addplot[color=black, line width=1.5pt] table from \teff;
	\pgfplotstableread{../data/estimativa_ctc_interface_01_conductance_03_stdev_05.dat} 
	\teff
	\addplot[only marks, color=blue,mark=o,mark options={mark size=1.5pt}] table from \teff;
	\pgfplotstableread{../data/estimativa_ctc_interface_02_conductance_03_stdev_05.dat} 
	\teff
	\addplot[only marks,color=red,mark=square,mark options={mark size=1.5pt}] table from \teff;
	\pgfplotstableread{../data/estimativa_ctc_interface_03_conductance_03_stdev_05.dat} 
	\teff
	\addplot[only marks,color=gray,mark=triangle,mark options={mark size=1.5pt}] table from \teff;
	\end{axis}
	\end{tikzpicture}
	%\caption*{(#4) Profile #3}
	%\end{minipage}
	\caption{Comparison of estimated TCC profile 3 for interface geometry 1 ($\textcolor{blue}{\ocircle}$), interface geometry 2 ($\textcolor{red}{\square}$) and interface geometry 3 ($\textcolor{gray}{\triangle}$), and the exact profile ($\text{--}$), using $\sigma = \text{0.5}\celsius$}
	\label{unlabeled_33}
\end{figure}

\noindent{\fontfamily{phv}\selectfont \textbf{Instability of estimatives and stop criteria for summations}}

It could be seen in this work that the expressions for estimation of temperature jump and heat flux across the contact interface in terms of the Reciprocity Functionals (equations \eqref{resultado_1} and \eqref{resultado_2}) did not have a monotonic convergent behavior as the number of summing elements increased indefinitely. In fact, each simulated configuration of contact interface and expected thermal contact conductance had its own optimal number of terms, so that adding more elements to either of the summations up to that limit led to a progressive improvement of the calculated estimatives through equation \eqref{equacao_definicao_f_r_rep}; from that point on, as more terms were included in the summations, the estimatives diverged quickly, leading to unstable results. On the other hand, when lower noise levels were added to the synthetic temperatures, those optimal number of terms could be increased. 
